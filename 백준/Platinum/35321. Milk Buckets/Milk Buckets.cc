...
pair <int, int> a[MAX];
void run(){
    cin >> n; a[n + 1].x = MINF; seg.clear(n + 1);
    for(int i = 1;i <= n;i++) cin >> a[i].x, a[i].y = i;
    sort(a + 1, a + n + 1); reverse(a + 1, a + n + 1);
    int idx = 1; ll result = 0;
    
    while(idx <= n){
        vector <pair<int, int>> arr;
        arr.push_back(a[idx]);
        while(a[idx].x == a[idx + 1].x){
            arr.push_back(a[idx + 1]); idx++;
        }
        for(auto& [v, pos] : arr) seg.set(pos, 1);

        for(auto& [v, pos] : arr){
            ll l = pos - 1 - seg.query(1, pos - 1).v;
            ll r = n - pos - seg.query(pos + 1, n).v;
            result += min(l, r);
        }
