...
struct node{
    int l, r, k, idx;
}; node seg[MAX], arr[MAX];
vector <int> comp;
map <int, int> num;
int result[MAX];

void run(){
    cin >> n; sl.clear(2 * n); sr.clear(2 * n);
    for(int i = 1;i <= n;i++){
        int l, r, k; cin >> l >> r >> k;
        arr[i] = {l, r, k, i};
        seg[i] = {l, r, k, i};
    ...
    sort(all(comp)); num[0] = 0;
    comp.erase(unique(all(comp)), comp.end());
    for(int i = 0;i < comp.size();i++) num[comp[i]] = i + 1;

    sort(arr + 1, arr + n + 1, [](node& a, node& b){
        return a.k > b.k;
    });

    sort(seg + 1, seg + n + 1, [](node& a, node& b){
    ...
    for(int i = 1;i <= n;i++){
        auto&[l, r, k, j] = arr[i];
        while(idx <= n && seg[idx].r - seg[idx].l >= k){
            sl.set(num[seg[idx].l], 1);
            sr.set(num[seg[idx].r], 1);
            idx++;
        }

        auto lb = (--num.lower_bound(l + k))->y;
        auto ub = num.upper_bound(r - k)->y;
        result[j] = idx - 1;
        result[j] -= sr.query(1, lb).v;
        result[j] -= sl.query(ub, 2 * n).v;
        result[j]--;
    }
